...
def main(name, project, train, test, 
         epochs=1, isSaveModel=False, my_tags=None, mul_num=False, 
    ...
    for epoch in range(epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            ...
            with tf.GradientTape() as tape:
                ...
                evidence = model(tf.expand_dims(x_batch_train, axis=0), training=True)
                alpha = evidence+1
        ...
        print(f"エポック中の訓練一致率：{train_acc}")
        # エポックの終わりに訓練メトリクスを初期化
        train_acc_metric.reset_states()
        
        # 訓練の終わりに検証用データ（今回はテストデータ）の性能を見る
        for x_batch_val, y_batch_val in val_dataset:
            val_evidence = model(tf.expand_dims(x_batch_val, axis=0), training=False)
...
if __name__ == '__main__':   
    ...
    for test_id in range(9):
        ...
        main(name = "edl", project = "edl",
             train=train, test=test, epochs=100, isSaveModel=True, mul_num=MUL_NUM,

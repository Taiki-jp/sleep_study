...
# pca
def make_pca(data=None, pse_data=True, path=None, show_data=True, data_type=None,
             test_id=0):
    if pse_data:
    ...
    if show_data:
        ...
        mappable = ax.scatter(X_reduced[:, 0], X_reduced[:, 1], c=digits.target)
...
def make_t_sne(data=None, pse_data=True, path=None, show_data=True):
    if pse_data:
        ...
        X_reduced = TSNE(n_components=2,
                         random_state=0).fit_transform(digits.data)
        # 圧縮後の次元を確認
...
def make_parallel_graph(targets, datas, comp_types):
    ...
    fig = plt.figure()
    for i, (target, data, comp_type) in enumerate(zip(targets, datas, comp_types)):
        # 表示は(1, len(data))の形
...
if __name__ == "__main__":
    pca_data, pca_target = make_pca(show_data=False, pse_data=True, data_type='sleep')
    t_sne_data, t_sne_target = make_t_sne(show_data=False, pse_data=True)
    compression_types = ("pca", "t-sne")
    make_parallel_graph(targets=(pca_target, t_sne_target),

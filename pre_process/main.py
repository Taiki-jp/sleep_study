...
target_folders = FR.my_env.set_raw_folder_path(
    is_normal=True, is_previous=True
)
utils = Utils(file_reader=FR)

for name in FR.sl.added_name_list:
    _, name = os.path.split(name)
    print(PyColor.GREEN, PyColor.FLASH, f" *** {name} を開始します ***", PyColor.END)
    records = list()
    ...
    records.append(
        CD.makeSpectrum(
            tanita.df, psg.df, kernel_size=1024, stride=STRIDE, fit_pos=FIT_POS
        )
    )
    utils.dump_with_pickle(records, name, data_type=DATA_TYPE, fit_pos=FIT_POS)

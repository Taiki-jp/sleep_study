# ================================================ #
# *            ライブラリのインポート
# ================================================ #

from utils import Utils
from my_setting import *
SetsPath().set()
from tanita_reader import TanitaReader
from psg_reader import PsgReader
from subjectsList import SubjectsList
from create_data import CreateData
from tqdm import tqdm

# ================================================ #
#            *クラスオブジェクトの生成
# ================================================ #

m_findsDir = FindsDir("sleep")
m_subjectsList = SubjectsList()
m_createData = CreateData()
m_utils = Utils()

# ================================================ #
#            *各被験者でループを回す 
# ================================================ #


for _, name in enumerate(tqdm(m_subjectsList.nameList)):
    records = list()
    m_tanitaReader = TanitaReader(name)
    m_psgReader = PsgReader(name)
    m_tanitaReader.readCsv()
    m_psgReader.readCsv()
    records.append(m_createData.makeCepstrogram(m_tanitaReader.df, m_psgReader.df))
    m_utils.dumpWithPickle(records, m_findsDir, name)

# ================================================ #
#            *個人ごとにループを回す
# ================================================ #
"""
records = list()
name = m_subjectsList.nameList[0]
m_tanitaReader = TanitaReader(name)
m_psgReader = PsgReader(name)
m_tanitaReader.readCsv()
m_psgReader.readCsv()
records.append(m_createData.makeSpectrogram(m_tanitaReader.df, m_psgReader.df))
"""
# ================================================ #
#                  *データの保存
# ================================================ #

#m_utils.dumpWithPickle(records, m_findsDir, m_subjectsList.nameList[0])

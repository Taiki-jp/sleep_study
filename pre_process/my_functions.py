...
def makeLongSpectroGram(data, hour):
    ...
    if type(hour) == int:
        ...
        sys.exit(1)
    del start_time
    target_list = [record for record in data if datetime.datetime.strptime(record.time, "%H:%M:%S") < end_time]
...
def show2dImage(x_label, 
    ...
    if path:
        ...
        plt.savefig(os.path.join(os.environ["SLEEP"], "figures", "tmp", "tmp.png"))
...
if __name__ == '__main__':
    ...
    for name in name_dict:
        ...
        hour = "all"
        (spectrogram, ss_list, time_list) = makeLongSpectroGram(data=records, hour=hour)
        # >> spectrogram.shape = (14464, 512) : len(ss_list) = 113 : len(time_list) = 113
        ...
        file_path = os.path.join(os.environ['SLEEP'],"figures","tmp",name+"_ceps"+".png")
        show2dImage(x_label="Time",
                    y_label="Frequency",
